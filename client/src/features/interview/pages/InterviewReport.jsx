...
//           <div className="h-px bg-[#1a2235] my-6" />

//           <div className="flex-1">
//             <button
//               onClick={() => getResumePdf(interviewId)}
//               className="flex items-center justify-center gap-2 w-full px-3 py-2.5 cursor-pointer text-sm font-medium text-white bg-gradient-to-r from-indigo-500 to-violet-500 rounded-lg border-0 shadow-lg shadow-indigo-500/30 hover:from-indigo-600 hover:to-violet-600 hover:shadow-indigo-500/40 transition-all"
//             >
//               <svg
//                 style={{ height: "0.8rem" }}
//                 xmlns="http://www.w3.org/2000/svg"
...
function QuestionCard({ item, index }) {
  ...
  return (
    <div className="bg-gray-800 p-1 border border-gray-700 rounded-xl mb-0 overflow-hidden shadow-sm">
      <button
        onClick={() => setOpen(!open)}
        className="w-full text-left px-4 sm:px-5 py-3.5 sm:py-4 flex items-start gap-3 cursor-pointer bg-gray-800 hover:bg-[#262b38] transition-colors"
      >
        <span className="mt-0.5 w-6 h-6 rounded-full bg-indigo-50 text-indigo-600 text-xs font-semibold flex items-center justify-center flex-shrink-0">
          {index + 1}
        </span>
      ...
      {open && (
        <div className="px-4 sm:px-5 pb-4 space-y-3 border-t border-slate-50">
          <div className="flex gap-2 pt-3">
            <span className="text-indigo-400 text-sm flex-shrink-0">💡</span>
            <p className="text-xs text-gray-300 leading-relaxed">
              {item.intention}
  ...
  return (
    <div className="bg-gray-800 border border-gray-700 rounded-2xl p-4 sm:p-6">
      <div className="flex items-center gap-4 sm:gap-5 flex-wrap">
        {/* Avatar with online dot */}
        <div className="relative flex-shrink-0">
          <div className="w-[64px] h-[64px] sm:w-[72px] sm:h-[72px] rounded-[18px] overflow-hidden border-2 border-indigo-700">
            <img
              src="https://thumbs.dreamstime.com/b/student-avatar-illustration-user-profile-icon-youth-avatar-student-avatar-illustration-simple-cartoon-user-portrait-user-profile-276214145.jpg"
              alt={`${candidate.name ?? "Candidate"} avatar`}
              className="w-full h-full object-cover"
        ...
        <div className="flex-1 min-w-0">
          ...
          <h2 className="text-lg sm:text-[22px] font-semibold text-white leading-tight mb-2 break-words">
            {candidate.name || "Not available"}
          </h2>
          <div className="flex items-center gap-2.5 flex-wrap">
            <span className="flex items-center gap-1.5 text-[13px] text-gray-400">
            ...
            >
              <div className="flex items-start justify-between gap-3 flex-wrap">
                <div className="min-w-0">
                  ...
                  <p className="text-xs text-gray-400 mt-0.5 break-words">{exp.company}</p>
                </div>
                <PeriodBadge period={exp.period} />
              </div>
              {exp.technologies?.length > 0 && (
    ...
    <div className="bg-gray-800 border border-gray-700 rounded-2xl p-4 sm:p-5">
      ...
      <div className="space-y-3">
        {projects.map((proj, i) => (
          ...
          >
            <div className="flex items-start justify-between gap-2 mb-1.5">
              <p className="text-sm font-medium text-white break-words">{proj.name}</p>
              <span className="text-gray-600 text-xs flex-shrink-0 mt-0.5">
                ↗
              </span>
            </div>
        ...
        {education.map((e, i) => (
          <div key={i} className="flex items-start gap-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-gray-400 mt-0.5 break-words">{e.institute}</p>
              <PeriodBadge period={e.year} />
            </div>
          </div>
        ))}
...
const PreparationDay = ({ day }) => (
  <div
    className="flex flex-col gap-2 py-3 pl-10 sm:pl-14 relative
    before:content-[''] before:absolute before:left-[17px] sm:before:left-[21px] before:top-[1.05rem]
    before:w-[14px] before:h-[14px] before:rounded-full
    before:bg-[#161b22] before:border-2 before:border-[#ff2d78]"
  >
...
const InterviewReport = () => {
  ...
  return (
    /* Page shell */
    <div className="w-full min-h-screen p-2 sm:p-4 bg-[#0d1117] text-[#e6edf3] font-[system-ui,-apple-system,BlinkMacSystemFont,'Segoe_UI',Roboto,sans-serif] flex items-stretch box-border [&::-webkit-scrollbar]:hidden">
      {/* 3-column card on desktop, stacked on mobile/tablet */}
      <div className="flex flex-col lg:flex-row w-full mx-auto bg-[#161b22] border border-[#2a3348] rounded-2xl lg:justify-between overflow-hidden">
        {/* ── Nav (horizontal scroll tabs on mobile/tablet, vertical column on desktop) ── */}
        <nav className="w-full lg:w-[220px] flex-shrink-0 px-3 sm:px-4 py-3 lg:py-7 flex flex-col gap-2 lg:gap-1 lg:justify-between">
          <div>
            <p className="text-[0.85rem] lg:text-[0.9rem] font-semibold uppercase tracking-[0.05em] text-[#b6bfca] px-1 lg:px-3 mb-2 lg:mb-3">
              Upskale AI
            </p>
            <div className="flex flex-row gap-1 overflow-x-auto lg:flex-col lg:overflow-visible [&::-webkit-scrollbar]:hidden -mx-1 px-1 lg:mx-0 lg:px-0">
              {NAV_ITEMS.map((item) => (
                <button
                  key={item.id}
                  onClick={() => setActiveNav(item.id)}
                  className={`flex items-center gap-[0.6rem] w-auto lg:w-full flex-shrink-0 px-3 py-[0.55rem] lg:py-[0.65rem] bg-transparent border-0 rounded-lg text-sm cursor-pointer text-left whitespace-nowrap transition-colors duration-150
                    ${
                      activeNav === item.id
                        ? "bg-[rgba(255,45,120,0.1)] text-[#ffffff] [&_svg]:stroke-[#ffffff]"
                        : "text-[#7d8590] hover:bg-[#191d28] hover:text-[#e6edf3]"
        ...
        {/* Divider — horizontal line on mobile/tablet, vertical on desktop */}
        <div className="h-px w-full lg:h-auto lg:w-px bg-[#2a3348] flex-shrink-0" />

        {/* ── Center Content ── */}
        <main className="flex-1 px-4 sm:px-6 lg:px-8 py-5 lg:py-7 overflow-visible lg:overflow-y-auto max-h-none lg:max-h-[calc(100vh-3rem)] pb-8 lg:pb-20 [&::-webkit-scrollbar]:hidden">
          {activeNav === "overview" && <SectionOverview report={report} />}

          {/* Technical */}
          {activeNav === "technical" && (
            <section className="min-h-full">
              <div className="flex items-baseline gap-3 mb-6 pb-4 border-b border-[#2a3348] flex-wrap">
                <h2 className="m-0 text-[1.1rem] font-bold text-[#e6edf3]">
                  Technical Questions
                </h2>
                <span className="text-[0.8rem] text-[#7d8590] bg-[#1c2230] border border-[#2a3348] px-[0.6rem] py-[0.15rem] rounded-full">
          ...
          {activeNav === "plan" && (
            <section className="min-h-full">
              ...
              <div
                className="relative
                before:content-[''] before:absolute before:left-5 sm:before:left-7 before:top-0 before:bottom-0
                before:w-0.5 before:rounded-sm
                before:bg-gradient-to-b before:from-[#ff2d78] before:to-[rgba(255,45,120,0.1)]"
              >
                {report.preparationPlan.map((day) => (
        ...
        {/* ── Right Sidebar ── */}
        <aside className="w-full lg:w-[260px] flex-shrink-0 flex flex-col gap-0 bg-[#0d1117] lg:border-l border-t lg:border-t-0 border-[#1e2535] px-4 py-5">
          {/* Match Score */}
          <p className="text-[12px] font-semibold uppercase tracking-[.1em] text-[#4b5675] mb-3">
            Match Score
          </p>
